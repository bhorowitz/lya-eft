...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description=(
            "Stage-1 Sherwood prior extraction with the patched full Ivanov basis: "
            "fit P3D in four redshift bins, project to P1D, and fit linear b_O(b1) relations."
        )
    )
...
def fit_single_bin(
    *,
    ...
    starts = [np.clip(x0_b, lo_b + 1.0e-8, hi_b - 1.0e-8)]
    n_starts = max(int(args.n_starts), 1)
    spread = float(args.start_spread)
    for _ in range(n_starts - 1):
        trial = x0_b.copy()
        # Keep b1/b_eta close to the Stage-A manifold and perturb the nonlinear block.
        trial[0] += rng.normal(scale=0.03)
        trial[1] += rng.normal(scale=0.05)
        trial[2:] += rng.normal(scale=spread * np.maximum(np.abs(x0_b[2:]), 0.2), size=x0_b.size - 2)
        starts.append(np.clip(trial, lo_b + 1.0e-8, hi_b - 1.0e-8))
    ...
    if best_fit_b is None:
        raise RuntimeError("Stage-B multistart failed to produce any fit result.")

    fit_b = best_fit_b
    params_b = theta_to_params(fit_b.x)
    pred_p3d = model.evaluate_components(k_all, mu_all, params_b)["total"]
    pred_fit = model.evaluate_components(k_fit, mu_fit, params_b)["total"]
    ...
    return {
        ...
        "p3d_diag": {
            ...
            "nfev_stage_a": int(fit_a.nfev),
            "nfev_stage_b": int(fit_b.nfev),
        },
        "p1d_diag": {
            "chi2": chi2_1d,
            "chi2_dof": float(chi2_1d / dof_1d),
            "n_data": int(kp_fit.size),
            "C0_1d": c0_1d,
            "C2_1d": c2_1d,
        },
        "data": {
...
def make_p3d_residual_plot(
    *,
    ...
    fig.suptitle(fr"Full-basis Sherwood Residuals ($z={z:.1f}$)")
    fig.tight_layout()
...
def make_linear_prior_plot(
    *,
    ...
    fig.suptitle("Full-basis Linear Prior Fits: b_O(b1) = A_O b1 + B_O")
    fig.tight_layout()
    fig.savefig(out_path, dpi=150)
    plt.close(fig)
...
def main() -> int:
    ...
    run_paths = init_run_dir(cfg.run.output_root, tag="repro_2405_stage1_sherwood_full")
    ...
    for snapshot in snapshots:
        ...
        fit = fit_single_bin(cfg=cfg, z=z, p3d_path=p3d_path, p1d_path=p1d_path, args=args, rng=rng)
        ...
        make_p3d_residual_plot(
            k_all=fit["data"]["k_all"],
            mu_all=fit["data"]["mu_all"],
            p_data=fit["data"]["p3d_all"],
            p_model=fit["p3d_pred_all"],
            z=z,
            out_path=f_p3d,
        )
        make_p1d_projection_plot(
            kp=fit["p1d_k_fit"],
            p_data=fit["p1d_data_fit"],
            p_raw=fit["p1d_raw"],
            p_fit=fit["p1d_fit"],
            z=z,
            out_path=f_p1d,
        ...
        row = {
            ...
            "b_delta2": float(params.b_delta2),
            "b_G2": float(params.b_G2),
            "b_KK_par": float(params.b_KK_par),
            "b_delta_eta": float(params.b_delta_eta),
            "b_eta2": float(params.b_eta2),
            "b_Pi2_par": float(params.b_Pi2_par),
        ...
        np.savez(
            ...
            k_all=fit["data"]["k_all"],
            mu_all=fit["data"]["mu_all"],
            p3d_data=fit["data"]["p3d_all"],
            p3d_pred=fit["p3d_pred_all"],
            kp_fit=fit["p1d_k_fit"],
            p1d_data=fit["p1d_data_fit"],
            p1d_raw=fit["p1d_raw"],
            p1d_fit=fit["p1d_fit"],
            theta_stage_a=fit["theta_stage_a"],
            theta_stage_b=fit["theta_stage_b"],
        )
        ...
        prior_rows.append(
            {
                ...
                "fit_variant": "full_ivanov_stage1_repro",
            }
        )
    ...
    shutil.copy2(f_prior, fig_target / f"{f_prior.stem}_full{f_prior.suffix}")
    shutil.copy2(run_paths.logs_dir / "z_bin_fit_summary.csv", table_target / "z_bin_fit_summary_full.csv")
    shutil.copy2(
        run_paths.logs_dir / "sherwood_prior_linear_fits.csv",
        table_target / "sherwood_prior_linear_fits_full.csv",
    )
    ...
    summary = {
        ...
        "notes": [
            "This is the first multi-z Sherwood prior extraction run using the patched full-basis Ivanov model.",
            "The projection quadrature has been stabilized by shifting the internal azimuthal sampling off phi=0.",
            "This stage should be validated on Sherwood before being promoted into the SDSS reproduction path.",
        ],
    }
    write_json(run_paths.logs_dir / "summary.json", summary)
    print(f"2405 Stage-1 full-basis Sherwood reproduction complete: {run_paths.run_dir}")
    return 0
